...
var _ = Describe("OtelDBCluster Controller", func() {
	Context("When reconciling a resource", func() {
		...
		oteldbcluster := &dbv1alpha1.OtelDBCluster{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind OtelDBCluster")
			err := k8sClient.Get(ctx, typeNamespacedName, oteldbcluster)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance OtelDBCluster")
		...
		It("should successfully reconcile the resource", func() {
			...
			By("creating the ConfigMap, Services and StatefulSet")
			cm := &corev1.ConfigMap{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: resourceName + "-config", Namespace: resourceNamespace}, cm)).To(Succeed())
			Expect(cm.Data).To(HaveKey("config.yml"))

			peer := &corev1.Service{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: resourceName + "-peers", Namespace: resourceNamespace}, peer)).To(Succeed())
