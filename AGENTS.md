# oteldb-operator — AI Agent Guide

## What this is

A Kubernetes operator (Kubebuilder v4) that runs **clustered [oteldb](https://github.com/oteldb/oteldb)**
using the embedded [`github.com/oteldb/storage`](https://github.com/oteldb/storage) engine's cluster
mode. See `README.md` for the user-facing docs.

CRD: **`OtelDBCluster`** — `db.oteldb.io/v1alpha1` (short names `odb`, `oteldb`).

**Mental model.** oteldb is a single symmetric binary — every node ingests (OTLP / Prom
remote-write), serves all query APIs (PromQL/LogQL/TraceQL/Pyroscope), stores, and replicates.
There is no ingester/querier/compactor split. Clustering = the storage engine's L0 layer:
- **etcd** coordinates membership (leases), the HRW ring, and compaction claims.
- Nodes replicate the unflushed head to `RF` peers over HTTP on the peer port (default 7946).
- Durable tier **defaults to a per-node `file` backend** (one PVC per pod, shared-nothing, RF gives
  redundancy). Shared S3 is optional.

The operator renders this as **one StatefulSet** + a headless peer Service (stable pod FQDNs) + a
client Service + a shared config ConfigMap.

## Design decisions (do not silently reverse)

- **etcd is bring-your-own** (`spec.etcd.endpoints`, required). The operator does NOT manage etcd —
  its backup/DR/upgrade lifecycle belongs to a dedicated etcd operator. Dev-only etcd:
  `config/samples/etcd-dev.yaml`.
- **Durable tier defaults to `file`** (per-node PVC), not S3. S3 is opt-in
  (`spec.storage.backend: s3`); S3 credentials are injected via env from a Secret (AWS default
  chain), never written into the ConfigMap.
- **Per-pod ring identity is injected via env**, not per-pod config files: the StatefulSet shares
  one ConfigMap, and each pod gets `OTELDB_CLUSTER_ID=$(POD_NAME)` and
  `OTELDB_CLUSTER_ADDR=$(POD_NAME).<name>-peers.<ns>.svc.cluster.local:7946`. This relies on env
  overrides added to oteldb's config loader (`oteldb/cmd/oteldb/config.go`:
  `OTELDB_CLUSTER_ID/ADDR/ZONE/ZONE_FILE/ETCD`). If you change the injection, keep it consistent
  with that loader.

## Where the logic lives (`internal/controller/`)

- `oteldbcluster_controller.go` — reconcile loop, `apply` (CreateOrUpdate + owner refs, preserving
  server-managed fields like clusterIP and immutable volumeClaimTemplates), status phase/conditions.
- `config.go` — `renderConfig` turns the spec into oteldb's `config.yml`. The rendered keys must
  match oteldb's `Config` struct (`oteldb/cmd/oteldb/config.go`). Per-pod `cluster.id/addr/zone` are
  intentionally omitted here.
- `resources.go` — builders for the ConfigMap, headless + client Services, and StatefulSet
  (per-pod env, ports, probes, PVC).
- `naming.go` — derived names, labels, ports.

A config-hash pod annotation triggers a rollout when the rendered config changes.

## Reference repos (siblings, separate modules)

- `../oteldb` — the oteldb server. Config surface is `cmd/oteldb/config.go`; the image is
  `ghcr.io/oteldb/oteldb`. `defaultImage` in `naming.go` pins the version.
- `../storage` — the storage library. Cluster design is in `storage/ARCHITECTURE.md` §3i and
  `storage/cluster/`.

When the oteldb config schema changes, update `renderConfig` and the render tests together.
